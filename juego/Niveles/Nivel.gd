...
onready var actualizador_timer:Timer = $ActualizarTimer
...
func _on_spawn_orbital(enemigo:EnemigoOrbita)->void:
...
func crear_explosion(
	...
	numero:int = 1,
	intervalo:float = 0.0,
	rango_aleatorio:Vector2 = Vector2(0.0,0.0)
	)->void:
	for i in range(numero):
		var new_explosion : Node2D = explosion.instance()
		new_explosion.global_position = posicion + crear_posicion_aleatoria(rango_aleatorio.x,rango_aleatorio.y)
...
func _on_RestarNivel_timeout() -> void:
